...
public class VhdlRulesDefinition implements RulesDefinition {
  ...
  public void defineFromRessources(Context context, HbRessourceContext ressourceContext) {
    ...
    if (rulesetXML != null) {
      List<com.lintyservices.sonar.plugins.vhdlrc.rules.Rule> rules = new HandbookXmlParser().parseXML(rulesetXML);
      if (rules != null) {
        new ExampleAndFigureLoader(HANDBOOK_DIR).load(rules);
        for (com.lintyservices.sonar.plugins.vhdlrc.rules.Rule r : rules) {
      ...
      LOG.error("handbook not found in jar ressources, re-build with {}", RULESET_PATH);
    }
  }
  ...
  private void addParameters(NewRule nr, com.lintyservices.sonar.plugins.vhdlrc.rules.Rule r) {
  ...
  private void addMetadataTo(NewRule nr, com.lintyservices.sonar.plugins.vhdlrc.rules.Rule r) {
    nr
